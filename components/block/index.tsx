...
export const Block: React.FC<BlockProps> = ({
  ...
  const blockGeometry = useMemo(
    () => new THREE.BoxGeometry(width, BRICK_HEIGHT - LAYER_GAP, depth),
    [width, depth]
  );
  ...
  useFrame((state) => {
    if (isPlacing && brickRef.current && studRef.current) {
      ...
      const glowColor = isEraseHighlight
  ...
  const instanceLimit = useMemo(
    () => Math.max(width * depth, 100),
    [width, depth]
  );
    ...
    >
      <mesh ref={brickRef} geometry={blockGeometry} castShadow receiveShadow>
        <meshStandardMaterial
          ...
          metalness={0.1}
          emissive={
            isPlacing ? (isEraseHighlight ? "#ff0000" : "#ffff00") : "#000000"
          }
